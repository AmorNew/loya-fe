...
import { Unit, UpdateGroup, UpdateUnit } from '../reducers/collectionsReducer'
    ...
    endpoints: (builder) => ({
        ...
        filterUnits: builder.query({
            ...
            async onCacheEntryAdded(arg, { cacheDataLoaded, dispatch }) {
                ...
                const points = data.result.units.reduce(
                    (acc: Point.AsObject[], { device: { hw_id }, position }: Unit): Point.AsObject[] => {
                        if (position) {
                            const {
                                latitude,
                                longitude,
                                last_nav_data: { nsat, course, speed },
                                updated_at,
                            } = position
        ...
        filterGroups: builder.query({
            query: ({ text = '', order_by = 'name', order_direction = 'asc' }) => {
                return {
                    credentials: 'include',
                    url: `filterGroups`,
                    method: 'POST',
        ...
        linkUnitGroup: builder.mutation({
            query: ({ groupId, unitId }: { groupId: number; unitId: number }) => {
                return {
                    credentials: 'include',
                    url: `linkUnitGroup`,
                    method: 'POST',
...
export const selectAllUnits = (state: RootState) =>
    loyaBackendApi.endpoints.filterUnits.select(state.data.searchParams)(state)?.data?.result?.units || emptyUsers
...
export const selectAllGroups = (state: RootState) =>
    loyaBackendApi.endpoints.filterGroups.select({
        text: '',
        order_by: 'name',
        order_direction: 'asc',
