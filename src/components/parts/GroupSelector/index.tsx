...
export const GroupSelector = ({ object = {}, creatable = true, filter = false, onChange }: Props) => {
    ...
    const [value, setValue] = useState<Option[]>(
        object.groups?.map(({ id, name, type }: Group) => {
            const option = {
                value: id,
                label: name,
                type,
    ...
    const [trigger, result] = useLazyFilterGroupsQuery()
    ...
    const promiseOptions = (inputValue: string) =>
        new Promise<Option[]>((resolve) => {
            trigger({ text: inputValue }).then((res) => {
                const options = res.data.result.groups.reduce((acc: Option[], { id, name, type }: Group) => {
        ...
        switch (actionMeta.action) {
            ...
            case 'select-option':
                if (object && object.id && actionMeta.option?.value) {
                    linkTrigger({
                        unitId: object.id,
                        groupId: Number(actionMeta.option?.value),
                    })
                }
            ...
            case 'remove-value':
                if (object && object.id && actionMeta.removedValue?.value) {
                    unlinkTrigger({
                        unitId: object.id,
                        groupId: Number(actionMeta.removedValue?.value),
                    })
                }
            ...
            return (
                selecComponents.DropdownIndicator && (
                    <selecComponents.DropdownIndicator {...props}>
                        <Icon type='filter' color='grey' className={styles.filterIcon} />
                    </selecComponents.DropdownIndicator>
                )
            )
        }
        ...
        <>
            ...
            {creatable && (
                <Modal show={showModal} onClose={handleModalClose}>
                    <GroupForm newGroupName={newGroupName} onSubmit={handleFormSubmit} />
                </Modal>
            )}
        </>
    )
